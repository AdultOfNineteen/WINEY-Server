...
public interface ShopRepository extends JpaRepository<Shop, Long>, ShopCustomRepository {
    ...
    @Query(value = "SELECT"
        ...
        + "    SQRT(POW(:latitude - S.latitude, 2) + POW(:longitude - S.longitude, 2)) * 111.195 AS meter, \n"
        ...
        + "FROM\n"
        + "    Shop S\n"
        + "    INNER JOIN ShopBookMark BM ON S.id = BM.shopId and BM.userId = :userId \n"
        + "WHERE\n"
        + "    MBRContains(\n"
        + "        ST_GeomFromText(\n"
        + "            CONCAT(\n"
        ...
        + "            4326\n"
        + "        ),\n"
        + "ST_GeomFromText(CONCAT('POINT(', S.latitude, ' ', S.longitude, ')'), 4326)\n"
        + "    )\n"
        + "ORDER BY\n"
        + "    meter ASC;\n",
        nativeQuery = true)
    ...
    @Query(value = "SELECT"
        ...
        + "    S.shopType AS shopType, "
        + "    S.latitude AS latitude, S.longitude AS longitude, "
        + "    SQRT(POW(:latitude - S.latitude, 2) + POW(:longitude - S.longitude, 2)) * 111.195  * 1000 AS meter, \n"
        + "    IF((SELECT EXISTS (SELECT * FROM ShopBookMark SBM WHERE SBM.userId = :userId AND S.id = SBM.shopId)), 'true', 'false') AS 'bookMark',\n"
        + "    (SELECT GROUP_CONCAT(SM.mood SEPARATOR ',') FROM ShopMood SM WHERE SM.shopId = S.id) AS shopMoods, "
        + "    S.name, S.businessHour, S.address, S.imgUrl, S.phone \n"
        + "FROM\n"
